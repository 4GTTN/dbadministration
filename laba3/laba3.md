...
Для анализа производительности запросов была использована таблица test. Были добавлены новые данные с помощью команды *INSERT INTO test (val) SELECT 'val_' || generate_series(1, 100000)*. Данная функция позволяет сгенерировать последовательность чисел в заданном диапазоне и автоматически создать большое количество строк, что удобно для тестирования производительности базы данных и анализа работы запросов на больших объёмах данных.

До создания индекса при выполнении запроса *EXPLAIN ANALYZE SELECT \* FROM test WHERE id = 45000;* использовался план выполнения Seq Scan (последовательное сканирование). PostgreSQL просматривает все строки таблицы одну за другой, проверяя для каждой строки выполнение условия id = 45000. Такой способ не использует никаких вспомогательных структур, поэтому время выполнения напрямую зависит от размера таблицы: чем больше строк, тем дольше выполняется запрос.

![Рисунок 3](photo/3.png)

После создания индекса с помощью команды *CREATE INDEX id ON test(val);* план выполнения изменился на Index Scan. Это означает, что PostgreSQL использует индекс для поиска нужных строк, что значительно сокращает время выполнения запроса.
...
Была реализована функция, которая принимает значения идентификатора и имени, после чего с помощью оператора *if* проверяет корректность переданных данных. В случае успешного прохождения проверки выполняется вставка новой записи в таблицу test, и функция возвращает сообщение «Запись добавлена». При не прохождении условия, функция возвращает сообщение об ошибке "Ошибка. id > 0 или name не пустой".
...
Для выполнения задания в таблицу test был добавлен новый столбец price, для которого будет проводиться проверка вводимых значение. После была создана триггерная функция, в которой реализовано правило проверки: значение цены не должно быть отрицательным. С помощью оператора RAISE EXCEPTION было реализовано прерывание выполнения операции вставки или обновления в зависимости от выполнения условия.
